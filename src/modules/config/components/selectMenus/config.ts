...
export const command = createCommand({
  ...
  data: new ComponentCommandBuilder().setName("config-select").setCustomId(/^config_select_module$/),
  ...
  execute: async (_client, interaction, commandIdData) => {
    ...
    if (selectedModule !== "leveling") {
      await interaction.reply({
        content: `Configuration for ${selectedModule} is coming soon!`,
        ephemeral: true,
      });
      return true;
    }
    ...
    let levellingConfig = await prisma.levelling.findUnique({
      where: { guildId: interaction.guildId },
    });

    if (!levellingConfig) {
      levellingConfig = await prisma.levelling.create({
